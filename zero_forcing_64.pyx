...
cpdef find_zero_forcing_set_connected(graph, int upper_bound=-1):
...
cpdef find_zero_forcing_set(graph, upper_bound=-1):
    graph = graph.copy()
    ...
    for g in connected_components:
        size, zfs, saved, perms, _ = find_zero_forcing_set_connected(g, upper_bound - len(current_zfs))
        if zfs:
...
# zfs_size, zfs_set, saved_calculations, num_automorphisms = find_zero_forcing_set(g)
